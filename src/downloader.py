...
from src.database import FILE_STATUS
...
class Downloader:
    ...
    def get_directory(self):
        ...
        if not exists(download_directory):
            self.logger.fail(
                f"Verilen '{download_directory}' geçerli bir klasör değil!"
            )
    ...
    def download_or_traverse(
        self, raw_html: str, destination_folder: str, db_instance, first_run
    ):
        try:
    ...
    def traverse_folder(
        self, folder_url, current_folder, new_folder_name, db_instance, first_run
    ):
        session = self.copy_session
    ...
    def download_file(
        self, file_url: str, destination_folder: str, cursor, db_instance, first_run
    ):
        session = self.copy_session
        ...
        match file_status:
            case FILE_STATUS.NEW:
                ...
                db_instance.add_file(
                    int(file_url[file_url.find("?g") + 2 :]), file_full_name
                )
